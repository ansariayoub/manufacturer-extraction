...
public class AnalyticsTransformationService : IAnalyticsTransformationService
{
    ...
    private static readonly string BaseSystemPrompt = """
        ...
        - If a row's value for the target period is zero, blank, or an extremely small floating-point artifact (e.g. 0.0000000001, effectively rounding error from spreadsheet formulas), treat it as no sale for that period — you may still extract the row with netSales null/0, but never invent a nonzero figure for it.
        ...
        - NEVER emit a row that has neither a customer/entity name nor any monetary value. Report titles, page headers, period banners and blank separator lines are not transactions — if a line gives you nothing but a title, skip it rather than emitting an all-null record.
        ...
        - SKIP TOTAL AND SUBTOTAL ROWS: a row whose first cell reads like a label rather than an identifier — "Total", "Totals for Rep No 90", "Totals for Territory", "Grand Total", "Subtotal", "Sum" — is an aggregate of rows you have already extracted. Never emit it, even though it carries large numbers.
